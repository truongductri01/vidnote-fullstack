...
import Tab from "../designComponents/Tab/Tab";
...
function NoteEditScreen() {
    ...
    const [tabId, setTabId] = useState<"summary" | "note">("summary");
    ...
    useEffect(() => {
        if (
            !selectedNote.video ||
            (selectedNote.video as any)?.id !== videoId
        ) {
            dispatch(setLoader(true));
            fetchYoutubeVideoByIdBackend(videoId as string)
                ...
                .catch((e) => {
                    dispatch(setLoader(false));
                    dispatch(
                        setToastError("Error while fetching youtube video" + e)
                    );
                });
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
            ...
            deleteNoteById(noteId, authInfo.accessToken)
                .then((res) => {
                    ...
                    if (res.ok) {
                        ...
                        dispatch(
                            setToastError("Something wrong happens, try again!")
                        );
                    }
                })
                .catch((e) => {
